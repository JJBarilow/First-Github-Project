...
# Normalize column names (remove whitespace, lowercase, replace spaces with underscores)
def normalize_columns(df):
    df.columns = df.columns.str.strip().str.lower().str.replace(' ', '_')
    return df

# Dynamically scale the y-axis based on the data's max value with 10% padding
...
# Define app layout with tabs for different dashboard views
app.layout = html.Div([
...
def render_content(tab):
    try:
        ...
        if tab == 'tab1':
            ...
            fig_trend.update_xaxes(tickmode='linear', dtick=86400000.0, tickformat='%b %d', tickangle=45)
        ...
        # -------------------------------
            ...
            fig_success.update_yaxes(ticksuffix='%')
        ...
        # -------------------------------
            ...
            if incidents_clean.empty:
                return html.Div("No incident report data available.", style={'textAlign': 'center', 'marginTop': '40px'})
            ...
            fig_res_status = px.bar(res_status_counts, x='status', y='count')
            fig_res_status = set_yaxis_padding(fig_res_status, 'count', res_status_counts)

            # Format report_time for table
            def format_report_time(dt):
            ...
            open_incidents_table = html.Table([
                ...
                html.Tbody([
                    html.Tr([html.Td(format_report_time(open_incidents.iloc[i][col]) if col == 'report_time' else open_incidents.iloc[i][col])
                             for col in ['incident_id', 'report_time', 'category', 'detected_by', 'resolution_status']])
                    for i in range(len(open_incidents))
                ])
            ], style={'width': '100%', 'border': '1px solid #ddd', 'borderCollapse': 'collapse', 'marginTop': '20px'})

            return html.Div([
                html.H3("Vulnerability & Compliance", style={'textAlign': 'Left', 'marginBottom': '20px'}),

                html.Div([
                    html.H4("Incident Counts by Category"),
                    html.P("This chart shows how many incidents are reported in each category.",
                           style={'fontSize': '13px', 'color': '#666', 'marginTop': '-10px', 'marginBottom': '10px', 'textAlign': 'center'}),
                    dcc.Graph(figure=fig_cat),
                ], style={'marginBottom': '40px'}),

                html.Div([
                    html.H4("Incident Resolution Status"),
                    html.P("Shows the distribution of incident resolution statuses.",
                           style={'fontSize': '13px', 'color': '#666', 'marginTop': '-10px', 'marginBottom': '10px', 'textAlign': 'center'}),
